...
try:
    # Carregar as configurações do arquivo TOML
    config = toml.load(config_path)
    data_path = config["files"]["data_path"]  # Ajuste para pegar o caminho do arquivo
    ...
    caminho_arquivo = data_path  # Usar o caminho carregado do TOML
    ...
    # Calcular os KPIs
    impressoes_total = df_filtrado["impressões"].sum()
    cliques_total = df_filtrado["cliques"].sum()
    ctr_total = (cliques_total / impressoes_total) * 100 if impressoes_total > 0 else 0
    investimento_total = df_filtrado["investimento"].sum()
    cpc_total = investimento_total / cliques_total if cliques_total > 0 else 0
    cpm_total = (investimento_total / impressoes_total * 1000) if impressoes_total > 0 else 0
    ...
    with col1:
        st.metric(label="Impressões", value=f"{impressoes_total:,.0f}")
    ...
    grafico = alt.Chart(df_melted).mark_bar(size=20).encode(
    ...
    chart = alt.Chart(df_soma).mark_bar(size=50).encode(
    ...
    print("Arquivo TOML carregado com sucesso!")
    print(f"Caminho do arquivo: {data_path}")
    
    # Verifica se o arquivo existe
    if os.path.exists(data_path):
        print("✅ O arquivo Excel foi encontrado!")
    else:
        print("❌ O arquivo Excel NÃO foi encontrado. Verifique o caminho.")
except Exception as e:
    print("❌ Erro ao carregar o arquivo TOML:", e)
